...
const ItemCard = ({ item, customerId }) => {
    ...
    return (
        <div className="bg-orange-50 p-4 rounded-lg shadow-md hover:bg-orange-100 transition relative cursor-pointer group" onClick={() => navigate(`/item/details/${item._id}`)}>
            <ToastContainer theme="light" position="top-right" autoClose={4000} />
            <div className="relative">
                <img
                    src={`http://localhost:3000/uploads/${item.image}`}
                    alt={item.name}
                    className="w-full h-48 object-cover rounded-md transition duration-300 group-hover:opacity-70 cursor-pointer"
                />
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex space-x-3 opacity-0 group-hover:opacity-100 transition duration-300">
                    <button
                        className="bg-orange-50 text-orange-500 border border-gray-300 p-3 rounded-lg shadow-md hover:bg-orange-500 hover:text-white transition"
                        onClick={(e) => {
                            e.stopPropagation();
                            setModalIsOpen(true);
                        }}
                    ...
                    <button
                        className={`bg-orange-50 p-3 rounded-lg shadow-md ${isWishlisted ? 'text-red-500 hover:bg-red-600 hover:text-white' : 'text-orange-500 hover:bg-orange-500 hover:text-white'} transition`}
                        onClick={(e) => {
                            e.stopPropagation();
                            toggleWishlist();
                        }}
            ...
            <div className="flex justify-between items-center mt-4">
                <h3 className="text-base font-semibold text-gray-800 cursor-pointer hover:text-orange-500 transition">
                    {item.name}
                </h3>
            </div>
            <span className="font-bold text-lg mt-2 text-orange-500 block">Rs {item.price}</span>
            <div className="flex items-center mt-4 space-x-2">
                <div className="flex items-center border border-gray-300 rounded-md">
                    ...
                    <span className="w-12 h-10 flex items-center justify-center text-center text-gray-800 bg-orange-50">
                        {quantity}
                    </span>
                    <button
                ...
                <button
                    className="border border-orange-500 text-orange-500 font-medium px-4 py-2 rounded-lg hover:bg-orange-500 hover:text-white transition"
                    onClick={(e) => {
                        e.stopPropagation();
                        addToCart();
                    }}
            ...
            <Modal
                ...
                className="fixed bg-orange-50 p-6 rounded-lg shadow-md max-w-md w-full mx-auto"
                overlayClassName="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[1000]"
            >
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-orange-500">{item.name}</h2>
                    <button
                        onClick={() => setModalIsOpen(false)}
                        className="text-orange-500 hover:text-[#009f4e] transition"
                    >
                        <X size={24} />
                    </button>
                </div>
                <div className="relative">
                    ...
                    />
                    <button
                        className={`absolute bottom-2 right-2 bg-orange-50 p-2 rounded-full shadow-md ${isWishlisted ? 'text-red-500 hover:bg-red-600 hover:text-white' : 'text-orange-500 hover:bg-orange-500 hover:text-white'} transition`}
                ...
                <span className="font-bold text-lg text-orange-500 block mb-4">Rs {item.price}</span>
                <div className="flex items-center justify-between space-x-4">
                    <div className="flex items-center border border-gray-300 rounded-md">
                        <button
                            className="bg-black text-white px-4 py-2 hover:bg-orange-black transition"
                        ...
                        <button
                            className="bg-black text-white px-4 py-2 hover:bg-black transition"
                    ...
                    <button
                        className="border border-orange-500 text-orange-500 font-medium px-4 py-2 rounded-lg flex-1 hover:bg-orange-500 hover:text-white transition"
                        onClick={addToCart}
                        disabled={addToCartMutation.isLoading || isCsrfLoading}
                    >
                        {addToCartMutation.isLoading ? 'Adding...' : 'Add to cart'}
